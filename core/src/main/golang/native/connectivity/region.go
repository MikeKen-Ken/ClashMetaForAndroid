...
// DefaultCustomProxyOrder 与桌面端 profiles.customProxyOrder 默认一致
...
// SortNamesByRegionAndConnectivity 地区顺序不变，同地区内按成功次数降序
func SortNamesByRegionAndConnectivity(names []string, customOrder []string) []string {
	if len(names) <= 1 {
		return names
	}
	...
	keys := make([]nameSortKey, len(names))
	for i, name := range names {
		flag := ResolveProxyFlag(name)
		keys[i] = nameSortKey{
			index:        i,
			groupOrder:   resolveGroupOrder(flag, customOrder, fallbackOrder, &nextFallback),
			successCount: GetSuccessCount(name),
		}
	}
