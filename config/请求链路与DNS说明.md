...
> 远程规则集（`ruleset/*.mrs`、自建 `c-*` 如 `c-wl-dir`/`c-dir` 等 `.mrs`）是否包含具体域名，需以实际 ruleset 与运行日志为准。
...
2. `c-proc-rej` → **REJECT**
3. `c-wl-dir` → **⬆️ DIRECT**
4. UDP + `c-wl-pxy` → **🔀 UDP**；否则 `c-wl-pxy` → **🔀**
5. `c-proc-wl-dir` → **⬆️ DIRECT**（进程白名单直连）
6. `ads` → **REJECT**
7. `private` / `privateip`（no-resolve）→ **⬆️**
8. `c-proc-dir` → **⬆️ DIRECT**
9. UDP + `c-proc-pxy` → **🔀 UDP**；否则 `c-proc-pxy` → **🔀**
10. `applications` → **⬆️**
11. `c-hk` → **🇭🇰**（UDP 走 **🇭🇰 UDP**）
12. `c-nohk` → **🚫 🇭🇰**（UDP 走 **🚫 🇭🇰 UDP**）
13. `c-dir` → **⬆️**
...
**没有**写 `AND,NETWORK,UDP` 的规则（如 `cn`、`private`、`c-dir`）对 **TCP / UDP 一视同仁**，命中后走同一策略组（多为 **⬆️ DIRECT**）。
...
#### C. `c-pxy` 中的域名（仅 UDP 流量）

```text
规则 → AND,(UDP + c-pxy) → 🔀 UDP
...
### 7.3 自定义代理白名单 `api.example.com`（`c-wl-pxy`）
