...
1. `custome-reject` → **REJECT**
2. `custome-whitelist-direct` → **⬆️ DIRECT**
3. UDP + `custome-whitelist-proxy` → **🔀 UDP**；否则 `custome-whitelist-proxy` → **🔀**
4. `private` / `privateip`（no-resolve）→ **⬆️**
5. `trackerslist` / `ads` → **REJECT**
6. `custome-HK` → **🇭🇰**（UDP 走 **🇭🇰 UDP**）
7. `custome-noHK` → **🚫 🇭🇰**（UDP 走 **🚫 🇭🇰 UDP**）
8. `custome-proxy` → **🔀**（UDP 走 **🔀 UDP**）
9. `custome-direct` → **⬆️**
10. `ai` → **🚫 🇭🇰**（UDP 走 **🚫 🇭🇰 UDP**）
11. `networktest` / `applications` / `microsoft-cn` / `apple-cn` / `games-cn` → **⬆️**
12. `cn` → **⬆️**
13. `cnip`（no-resolve）→ **⬆️**
14. `NETWORK,UDP` → **↩️ UDP**
15. `MATCH` → **↩️ FALLBACK**
...
**没有**写 `AND,NETWORK,UDP` 的规则（如 `cn`、`private`、`custome-direct`）对 **TCP / UDP 一视同仁**，命中后走同一策略组（多为 **⬆️ DIRECT**）。

### 6.2 本配置中与 UDP 相关的规则一览

| 规则 | UDP 命中时 | TCP 命中时 |
|------|------------|------------|
| `custome-whitelist-proxy` | **🔀 UDP** | **🔀** |
| `custome-HK` | **🇭🇰 UDP** | **🇭🇰** |
| `custome-noHK` | **🚫 🇭🇰 UDP** | **🚫 🇭🇰** |
| `custome-proxy` | **🔀 UDP** | **🔀** |
| `ai` | **🚫 🇭🇰 UDP** | **🚫 🇭🇰** |
...
#### B. 国外游戏 / VoIP（域名不在 cn，未命中 custome-*）
...
#### C. `custome-proxy` 中的域名（仅 UDP 流量）

```text
规则 → AND,(UDP + custome-proxy) → 🔀 UDP
...
### 7.3 自定义代理白名单 `api.example.com`（`custome-whitelist-proxy`）
