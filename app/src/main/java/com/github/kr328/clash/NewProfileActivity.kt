...
class NewProfileActivity : BaseActivity<NewProfileDesign>() {
    ...
    private suspend fun launchProperties(uuid: UUID, autoCommit: Boolean = false) {
        val r = startActivityForResult(
            ActivityResultContracts.StartActivityForResult(),
            PropertiesActivity::class.intent
                .setUUID(uuid)
                .putExtra(Intents.EXTRA_AUTO_COMMIT, autoCommit)
        )
    ...
    private suspend fun createProfileByQrCode(url: String) {
        try {
            ...
            launchProperties(uuid, autoCommit = true)
        } catch (e: Exception) {
